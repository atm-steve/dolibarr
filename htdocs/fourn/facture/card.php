...
if ($action == 'create')
{
	...
	if ($societe->id > 0 && (!GETPOST('fac_rec', 'int') || !empty($invoice_predefined->frequency))) //BACKPORT - 10/01/2022 - Template supplier invoice
	{
		...
		if (!empty($conf->global->RELOAD_PAGE_ON_SUPPLIER_CHANGE))
		{
			...
			$(document).ready(function() {
				$("#socid").change(function() {
					var socid = $(this).val();
                    var fac_rec = $(\'#fac_rec\').val();  //BACKPORT - 10/01/2022 - Template supplier invoice
					// reload page
					window.location.href = "'.$_SERVER["PHP_SELF"].'?action=create&socid="+socid+"&fac_rec="+fac_rec;  //BACKPORT - 10/01/2022 - Template supplier invoice
				});
			});
			</script>';
		}
