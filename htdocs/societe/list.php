...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label))
{
   ...
   {
		if (! empty(dol_eval($extrafields->attribute_list[$key],1))) $arrayfields["ef.".$key]=array('label'=>$extrafields->attribute_label[$key], 'checked'=>(($extrafields->attribute_list[$key]<0)?0:1), 'position'=>$extrafields->attribute_pos[$key], 'enabled'=>(abs(dol_eval($extrafields->attribute_list[$key],1))!=3 && $extrafields->attribute_perms[$key]));
   }
}
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && ($search_all != '' || $search_cti != '') && $action != 'list') {
    ...
    // Spé C'Pro : Il faut aller sur la création d'une simulation avec ce socid
...
while ($i < min($num, $limit))
{
	...
	if (! empty($arrayfields['s.nom']['checked']))
	{
		...
		print '<td class="maxwidth200 nowrap">';
		if ($contextpage == 'poslist')
		{
		    print $obj->name;
		}
