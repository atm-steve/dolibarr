 ...
 * Copyright (C) 2019      Jean-Fraçois Ferry   <hello+jf@librethic.io>
 *
...
if (GETPOST("save"))
{
    ...
    $res+=dolibarr_set_const($db,'LAB_UNIT',trim(GETPOST("labUnit")),'chaine',0,'',$conf->entity);
	...
	if($status==1)
	{
		...
		switch ($value)
		{
			case (0):
				...
				dolibarr_del_const($db, "MAIN_MODULE_LABELPRINT_LABELS_2");
				break;
			case (1):
...
$labunit = dolibarr_get_const($db,"LAB_UNIT",$conf->entity);
...
while ($i < $nbofloop)
{
	...
	{
		print ajax_constantonoff('MAIN_MODULE_LABELPRINT_TABS_'.$i);
	}
	else{
...
$var=!$var;
print "<tr ".$bc[$var].">";
print "<td>".$langs->trans("ShowUnit")."</td>";
print '<td>';
print $html->selectyesno("labUnit",$labunit,1);
print '</td>';
print "</tr>";
