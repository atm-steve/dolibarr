...
// Generate Barcode
if ($action == 'genbarcode_all')
{
    $maxperinit=1000;
    
	$sql ="SELECT rowid, ref, fk_product_type, fk_barcode_type";
    $sql.=" FROM ".MAIN_DB_PREFIX."product";
    $sql.=" WHERE barcode IS NULL or barcode = ''";
    $sql.=$db->order("datec","ASC");
    $sql.=$db->plimit($maxperinit);

    dol_syslog("genbarcode_all", LOG_DEBUG);
    $resql=$db->query($sql);
    if ($resql)
    {
        ...
        while ($i < min($num,$maxperinit))
        {
            ...
            if ($obj)
                ...
                $object = new Labelprint($db);
                $object->fetch(0);
                $result = $object->generate_barcode($obj->rowid);

                if ($result > 0)
                {
                    $mesg .= '<font class="ok">'.$langs->trans("BarcodeGenerated").' : '.$obj->ref.'</font><br />';
                }
                else
                {
                    $mesg='<div class="error">'.$object->error.'</div>';
...
if ($resql)
{
	...
	if ($user->rights->produit->creer)
	{
        ...
        print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=generatebarcode_all">Missing barcodes</a>';
