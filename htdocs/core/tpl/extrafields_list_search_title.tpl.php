		...
		foreach ($extrafields->attributes[$extrafieldsobjectkey]['label'] as $key => $val)
		{
			if (!empty($arrayfields[$extrafieldsobjectprefix.$key]['checked']))
			{
				...
				if ($extrafields->attributes[$extrafieldsobjectkey]['type'][$key] == 'separate') {
					...
					print getTitleFieldOfList($extrafields->attributes[$extrafieldsobjectkey]['label'][$key], 0, $_SERVER["PHP_SELF"], $sortonfield, "", $param, ($align?'align="'.$align.'"':''), $sortfield, $sortorder)."\n";
				}
