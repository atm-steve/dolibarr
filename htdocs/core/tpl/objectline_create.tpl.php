	...
	if (! empty($conf->product->enabled) || ! empty($conf->service->enabled))
	{
		...
		if (empty($senderissupplier))
		{
			if (! empty($conf->global->ENTREPOT_EXTRA_STATUS))
			{
				...
				$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, 1, 2, '', 1, array(), $buyer->id, 0, 0, 'maxwidth300', 0, 'warehouseopen,warehouseinternal', GETPOST('combinations', 'array'));
			}
			else
			{
				$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, 1, 2, '', 1, array(), $buyer->id, 0, 0, 'maxwidth300', 0, '', GETPOST('combinations', 'array'));
			}
		}
		else
		{
