...
class modFacture extends DolibarrModules
{
	...
	/*	$this->export_sql_order[$r] =' GROUP BY ';
		foreach($this->export_fields_array[$r] as $k=>$v){
			if (strpos($k, '(') == false && strpos($k, 'one.rest') == false ) $this->export_sql_order[$r].=$k.','; // CUSTOM FOR GROUP CONCAT 
		}
		$this->export_sql_order[$r] = substr($this->export_sql_order[$r], 0, -1);*/
		$r++;
