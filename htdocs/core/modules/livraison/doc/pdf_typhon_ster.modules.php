...
/**
 *	\file       htdocs/core/modules/livraison/pdf/pdf_typhon_ster.modules.php
 *	\ingroup    livraison
 ...
 */
class pdf_typhon_ster extends ModelePDFDeliveryOrder
{
    var $db;
    var $name;
    var $description;
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblines ; $i++)
				{
					...
					$pdf->MultiCell($this->posxqtyremaining - $this->posxqty - 2, 3, $object->lines[$i]->qty_shipped, 0, 'R');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$larg_sign = ($this->page_largeur-$this->marge_gauche-$this->marge_droite)/2;
		$pdf->Rect($this->marge_gauche, $posy + 1, $larg_sign, 25);
		$pdf->SetXY($this->marge_gauche + 2, $posy + 2);
		$pdf->MultiCell($larg_sign,2, $outputlangs->trans("Date")." :",'','L');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell($this->posxqtyremaining - $this->posxqty, 2, $outputlangs->transnoentities("QtyShipped"),'','R');
		}
		
		// Qty Remaining
		$pdf->line($this->posxqtyremaining-1, $tab_top, $this->posxqtyremaining-1, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxqtyremaining-1, $tab_top+1);
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxqtyremaining, 2, $outputlangs->transnoentities("KeepToShip"),'','R');
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$origin 	= 'commande';
		$origin_id 	= $expedition->linkedObjects['commande'][0]->id;
		...
		if ($conf->$origin->enabled)
		{
			...
			if ($result >= 0)
			{
				...
				$pdf->MultiCell(60, 2, $outputlangs->transnoentities("Date")." : ".dol_print_date($linkedobject->date,"daytext",false,$outputlangs,true), 0, 'R');
			}
		}
