...
function ajax_combobox($htmlname, $events=array(), $minLengthToAutocomplete=0, $forcefocus=0, $widthTypeOfAutocomplete='resolve', $htmlid='')
{
    ...
    $tmpplugin='select2';
    $msg="\n".'<!-- JS CODE TO ENABLE '.$tmpplugin.' for id = '.$htmlid.' -->
	...
	if (is_array($events) && count($events))    // If an array of js events to do were provided.
	{
		$msg.= '
			...
			function runJsCodeForEvent'.$htmlid.'(obj) {
			    ...
			    console.log("Run runJsCodeForEvent-'.$htmlid.' from ajax_combobox id="+id+" method="+method+" showempty="+showempty+" url="+url+" htmlname="+htmlname);
				$.getJSON(url,
						{
							action: method,
							id: id,
