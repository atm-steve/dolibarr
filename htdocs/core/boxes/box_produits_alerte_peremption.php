 ...
 */
class box_produits_alerte_peremption extends ModeleBoxes
{
	var $boxcode="productsalertperemption";
	var $boximg="object_product";
	var $boxlabel="BoxProductsAlertPeremption";
	...
	function loadBox($max=5)
	{
		...
		$this->info_box_head = array('text' => $langs->trans("BoxProductsAlertPeremption",$max));
		...
		if (($user->rights->produit->lire || $user->rights->service->lire) && $user->rights->stock->lire)
		{
			...
			$sql.="pl.sellby, ";
			$sql.="SUM(b.qty) qty, ";
			$sql.="GROUP_CONCAT(e.lieu) Entrepot ";
			$sql.="FROM "
				.MAIN_DB_PREFIX."entrepot as e, "
				.MAIN_DB_PREFIX."product as p, "
				.MAIN_DB_PREFIX."product_batch as b, "
				.MAIN_DB_PREFIX."product_stock as ps,  "
				.MAIN_DB_PREFIX."product_lot as pl";
			$sql.= " 
				WHERE p.rowid = ps.fk_product 
				AND ps.fk_entrepot = e.rowid 
				AND pl.fk_product = p.rowid 
				AND e.rowid <> 40
				AND b.fk_product_stock = ps.rowid
				AND pl.batch = b.batch 
				AND e.statut = 1 
				AND b.qty > 0 AND pl.sellby Is Not Null 
				AND pl.sellby < DATE_ADD(CURDATE(), INTERVAL 3 MONTH) 
				GROUP BY  p.ref, pl.batch, pl.sellby, ps.rowid
				";

    		if (is_object($hookmanager))
    		{
			    $parameters=array('productsalertperemption'=>1);
    		    $reshook=$hookmanager->executeHooks('printFieldListWhere',$parameters);    // Note that $action and $object may have been modified by hook
    		    $sql.=$hookmanager->resPrint;
    		}
		    $sql.= $db->order('b.sellby', 'ASC');
			$sql.= $db->plimit($max, 0);
			...
			if ($result)
			{
                ...
                while ($line < $num) {
					...
					$productstatic->batch = $objp->batch;
					$productstatic->sellby = $objp->sellby;
					$productstatic->qty = $objp->Qty;
					...
					$this->info_box_contents[$line][] = array(
					    'td' => 'align="right"',
                    'text' => $objp->batch . ' ('.$objp->sellby. ') - Qte : '.$objp->qty,
					'text2'=>img_warning($langs->transnoentitiesnoconv("AlertePeremption3mois", $objp->seuil_stock_alerte)));
                ...
                if ($num==0)
                    $this->info_box_contents[$line][0] = array(
                        'td' => 'align="center"',
                        'text'=>$langs->trans("AlertePeremption3mois"),
                    );
