...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'skilldetcard'; // To manage different context of search
...
// Part to create
if ($action == 'create') {
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Skilldet")), '', 'object_'.$object->picto);
	...
	if (!empty($fk_skill)){
		print '<input type="hidden" name="fk_skill" value="'.$fk_skill.'">';
	}
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Skilldet"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteSkilldet'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action != 'presend') {
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('skilldet'));
