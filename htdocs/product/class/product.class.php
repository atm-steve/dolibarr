...
class Product extends CommonObject
{
    ...
    */
    ...
    public $fields = array(
		...
		'managed_in_stock'	=>array('type'=>'integer', 'label'=>'managed_in_stock', 'enabled'=>1, 'visible'=>1, 'default'=>1, 'notnull'=>1, 'index'=>1, 'position'=>502),
    ...
    public function create($user, $notrigger = 0)
    {
		...
		if (empty($this->managed_in_stock)) {
			$this->managed_in_stock = 0;
		}
        ...
        if ($result >= 0) {
            ...
            if ($result) {
                ...
                if ($obj->nb == 0) {
					...
					$sql .= " " . (empty($this->managed_in_stock) ? 0 : $this->managed_in_stock)  ;
                    $sql .= ")";
    ...
    public function update($id, $user, $notrigger = false, $action = 'update', $updatetype = false)
    {
        ...
        if ($result >= 0) {
			...
			$sql .= ", managed_in_stock = ".(empty($this->managed_in_stock) ? '0' : $this->managed_in_stock );
    ...
    public function fetch($id = '', $ref = '', $ref_ext = '', $barcode = '', $ignore_expression = 0, $ignore_price_load = 0, $ignore_lang_load = 0)
    {
        ...
        $sql .= " fk_price_expression, price_autogen, managed_in_stock";
        $sql .= " FROM ".MAIN_DB_PREFIX."product";
        if ($id) {
        ...
        if ($resql) {
            ...
            if ($this->db->num_rows($resql) > 0) {
				...
				$this->managed_in_stock  		  = $obj->managed_in_stock;
                $this->db->free($resql);
