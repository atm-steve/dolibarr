...
class MouvementStock extends CommonObject
{
    ...
    public function _create($user, $fk_product, $entrepot_id, $qty, $type, $price = 0, $label = '', $inventorycode = '', $datem = '', $eatby = '', $sellby = '', $batch = '', $skip_batch = false, $id_product_batch = 0)
	{
		...
		if ($movestock && $entrepot_id > 0 && $product->managed_in_stock == Product::MANAGED_IN_STOCK)	// Change stock for current product, change for subproduct is done after
		{
		...
		{
            // Call trigger
			if ($product->managed_in_stock == Product::MANAGED_IN_STOCK ) {
