...
if ($action == 'create' && $user->rights->projet->creer)
{
    ...
    if ($conf->societe->enabled)
    {
        ...
        if (! empty($conf->global->PROJECT_FILTER_FOR_THIRDPARTY_LIST)) $filteronlist=$conf->global->PROJECT_FILTER_FOR_THIRDPARTY_LIST;
       	$text=$form->select_thirdparty_list(GETPOST('socid','int'), 'socid', $filteronlist, 'SelectThirdParty', 1, 1, array(), '', 0, 0, 'minwidth300');
...
elseif ($object->id > 0)
{
    ...
    if ($action == 'edit' && $userWrite > 0)
    {
        ...
        if ($conf->societe->enabled)
        {
    	    ...
    	    $filteronlist='';
    	    if (! empty($conf->global->PROJECT_FILTER_FOR_THIRDPARTY_LIST)) $filteronlist=$conf->global->PROJECT_FILTER_FOR_THIRDPARTY_LIST;
            $text=$form->select_thirdparty_list($object->thirdparty->id, 'socid', $filteronlist, 'None', 1,1, array(), '', 0, 0, 'minwidth300');
	        if (empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS) && empty($conf->dol_use_jmobile))
		    {
	            $texthelp=$langs->trans("IfNeedToUseOhterObjectKeepEmpty");
