...
class SupplierProposal extends CommonObject
{
    ...
    public function updateOrCreatePriceFournisseur($user)
    {
        ...
        foreach ($this->lines as $product)
        {
            ...
            $productsupplier->update_buyprice($product->qty, $product->subprice*$product->qty, $user, 'HT', $this->thirdparty, '', $ref_fourn, $product->tva_tx, 0, 0, 0, $product->info_bits,  '',  '',  array(),  '', $product->multicurrency_subprice,  'HT', $multicurrency_tx, $product->multicurrency_code,  '',  '',  '' );
        }
