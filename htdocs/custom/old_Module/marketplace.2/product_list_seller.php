...
$searchSeller = GETPOST('search_seller', 'int');
if ($user->societe_id > 0) {
    $searchSeller = $user->societe_id;
}
$fk_seller = GETPOST('fk_seller', 'int');
if ($fk_seller > 0) {
    $searchSeller = $fk_seller;
}
...
$arrayfields=array(
    ...
    'mp.fk_seller'=>array('label'=>$langs->trans("Seller"), 'checked'=>1, 'position'=>20),
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."marketplace_productseller as mp ON mp.fk_product = p.rowid";
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."societe as s ON mp.fk_seller = s.rowid";
...
// Marketplace : search by seller
if ($searchSeller > 0) {
    $sql .= " AND mp.fk_seller=".$db->escape($searchSeller);
}
...
if ($resql) {
    ...
    if ($fk_seller > 0) {
        ...
        dol_fiche_head($head, 'marketplaceproducts', $langs->trans("ThirdParty"), 0, 'company');
    ...
    // Marketplace seller
	if (! empty($arrayfields['mp.fk_seller']['checked'])) {
		print '<td class="liste_titre" align="left">';
		
		print $form->select_company($searchSeller, 'search_seller', '', 1);
		print '</td>';
	}
	// Barcode
    ...
    if (! empty($arrayfields['mp.fk_seller']['checked'])) {
        print_liste_field_titre($arrayfields['mp.fk_seller']['label'], $_SERVER["PHP_SELF"], "mp.fk_seller", "", $param, "", $sortfield, $sortorder);
    }
	if (! empty($arrayfields['p.barcode']['checked'])) {
