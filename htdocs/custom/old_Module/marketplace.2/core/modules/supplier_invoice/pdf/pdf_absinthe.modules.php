...
/**
 *	\file       marketplace/core/modules/supplier_invoice/pdf/pdf_absinthe.modules.php
 *	\ingroup    fournisseur
 *	\brief      Class file to generate the seller/supplier invoices with the absinthe model
 */
...
/**
 *	Class to generate the seller invoices with the absinthe model
 */
class pdf_absinthe extends ModelePDFSuppliersInvoices
{
    /**
    ...
    public function __construct($db)
    {
        ...
        $this->db = $db;
        $this->name = "absinthe";
        $this->description = $langs->trans('MarketPlaceSellersInvoiceModel');
    ...
    public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        if ($conf->fournisseur->facture->dir_output) {
            ...
            if (file_exists($dir)) {
                ...
                // Affiche calcul commission
                $posy = $this->_tableau_commissions($pdf, $object, $bottomlasttab, $outputlangs);
    ...
    public function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
    {
        ...
        // Load translation files required by the page
        $outputlangs->loadLangs(array("main", "orders", "companies", "bills", "marketplace@marketplace"));
        ...
        $pdf->SetXY($posx, $posy);
        $pdf->SetTextColor(0, 0, 60);
        $pdf->MultiCell(100, 3, $outputlangs->transnoentities("MarketPlaceSellerInvoiceTitle")." ".$outputlangs->convToOutputCharset($object->ref), '', 'R');
        $posy+=1;
        ...
        if ($object->ref_supplier) {
            ...
            $pdf->SetXY($posx, $posy);
            $pdf->SetTextColor(0, 0, 60);
            $pdf->MultiCell(100, 4, $outputlangs->transnoentities("MarketPlaceSellerRef")." : " . $object->ref_supplier, '', 'R');
            $posy+=1;
        }
        ...
        if ($showaddress) {
            ...
            $pdf->MultiCell(66, 5, $outputlangs->transnoentities("MarketPlaceInvoiceFor").":", 0, 'L');
            $pdf->SetXY($posx, $posy);
            ...
            // Invert recipient for marketplace
            $posx=$this->marge_gauche;
     ...
     *	@return int							<0 if KO, >0 if OK
     */
    public function _tableau_commissions(&$pdf, $object, $posy, $outputlangs)
    {
        // phpcs:enable
        global $conf;
        ...
        $tab3_posx = 10;
        $tab3_top = $posy + 30;
        $tab3_width = $this->page_largeur - $this->marge_gauche - $this->marge_droite;
        $tab3_height = 4;
        if ($this->page_largeur < 210) { // To work with US executive format
            $tab3_posx -= 20;
        }
        ...
        $pdf->SetFont('', 'B', $default_font_size - 2);
        $pdf->SetXY($tab3_posx, $tab3_top - 4);
        $pdf->MultiCell(60, 3, $outputlangs->transnoentities("MarketPlaceSalesSettlement"), 0, 'L', 0);

        //$pdf->line($tab3_posx, $tab3_top, $tab3_posx+$tab3_width, $tab3_top);

        $pdf->SetFont('', 'B', $default_font_size - 3);
        $pdf->SetXY($tab3_posx, $tab3_top);
        $pdf->MultiCell(40, 3, $outputlangs->transnoentities("Rate"), 0, 'L', 0);
        $pdf->SetXY($tab3_posx+41, $tab3_top);
        $pdf->MultiCell(25, 3, $outputlangs->transnoentities("AmountHT"), 0, 'R', 0);
        $pdf->SetXY($tab3_posx+66, $tab3_top);
        $pdf->MultiCell(25, 3, $outputlangs->transnoentities("AmountVAT"), 0, 'R', 0);
        $pdf->SetXY($tab3_posx+91, $tab3_top);
        $pdf->MultiCell(25, 3, $outputlangs->transnoentities("AmountTTC"), 0, 'R', 0);
        $pdf->SetXY($tab3_posx+116, $tab3_top);
        $pdf->MultiCell(25, 3, $outputlangs->transnoentities("Commission HT"), 0, 'R', 0);
        $pdf->SetXY($tab3_posx+141, $tab3_top);
        $pdf->MultiCell(25, 3, $outputlangs->transnoentities("TVA commission"), 0, 'R', 0);
        $pdf->SetXY($tab3_posx+166, $tab3_top);
        $pdf->MultiCell(25, 3, $outputlangs->transnoentities("Commission TTC"), 0, 'R', 0);
        ...
        $arraySales = $this->getSalesListForInvoice($object->id);

        if (is_array($arraySales) && count($arraySales) > 0) {
            $arraySalesByCollectionRate = array();

            foreach ($arraySales as $saleLine) {
                $arraySalesByCollectionRate[$saleLine['collection_rate']][$saleLine['tax_rate']]['amount_ht'] += $saleLine['price'];
                $arraySalesByCollectionRate[$saleLine['collection_rate']][$saleLine['tax_rate']]['tax_total'] += $saleLine['tax_total'];
                $arraySalesByCollectionRate[$saleLine['collection_rate']][$saleLine['tax_rate']]['collection_amount'] += $saleLine['collection_amount'];
                $arraySalesByCollectionRate[$saleLine['collection_rate']][$saleLine['tax_rate']]['retrocession_amount'] += $saleLine['retrocession_amount'];
            }
            ...
            $totalCollectionTVA = 0;

            $y+=3;
            foreach ($arraySalesByCollectionRate as $collectionRate => $collectionTaxes) {
                foreach ($collectionTaxes as $taxRate => $arraySale) {
                    $pdf->SetXY($tab3_posx, $tab3_top+$y);
                    $labelCollection = 'CA '.price($collectionRate, 0, $outputlangs, 1, 2, 2) .'% / TVA '.vatrate($taxRate, 1);
                    $pdf->MultiCell(40, 3, $labelCollection, 0, 'L', 0);
                    
                    $pdf->SetXY($tab3_posx+41, $tab3_top+$y);
                    $pdf->MultiCell(25, 3, price($arraySale['amount_ht'], 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
                    $totalAmountHT += $arraySale['amount_ht'];
                    
                    $pdf->SetXY($tab3_posx + 66, $tab3_top+$y);
                    $pdf->MultiCell(25, 3, price($arraySale['tax_total'], 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
                    $totalTax += $arraySale['tax_total'];

                    $pdf->SetXY($tab3_posx+91, $tab3_top+$y);
                    $amountTTC = $arraySale['amount_ht'] + $arraySale['tax_total'];
                    $pdf->MultiCell(25, 3, price($amountTTC, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
                    $totalAmountTTC += $amountTTC;
                    
                    $pdf->SetXY($tab3_posx+116, $tab3_top+$y);
                    $pdf->MultiCell(25, 3, price($arraySale['collection_amount'], 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
                    $totalAmountCollection += $arraySale['collection_amount'];

                    $pdf->SetXY($tab3_posx+141, $tab3_top+$y);
                    include_once DOL_DOCUMENT_ROOT . '/core/lib/price.lib.php';
                    $tabprice = calcul_price_total(1, $arraySale['collection_amount'], 0, $taxRate, 0, 0, 0, 'HT', $info_bits, $type, $this->thirdparty, $localtaxes_type, 100, $this->multicurrency_tx, $pu_ht_devise);
                    $total_ht  = $tabprice[0];
                    ...
                    $pdf->MultiCell(25, 3, price($total_tva, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
                    $totalCollectionTVA += $total_tva;

                    $pdf->SetXY($tab3_posx+166, $tab3_top+$y);
                    $collectionAmountTTC = $arraySale['collection_amount'] + $total_tva;
                    $pdf->MultiCell(25, 3, price($collectionAmountTTC, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
                    $totalCollectionTTC += $collectionAmountTTC;
            ...
            $pdf->MultiCell(40, 3, 'Total', 0, 'L', 0);
                    
            $pdf->SetXY($tab3_posx+41, $tab3_top+$y);
            $pdf->MultiCell(25, 3, price($totalAmountHT, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
            
            $pdf->SetXY($tab3_posx+66, $tab3_top+$y);
            $pdf->MultiCell(25, 3, price($totalTax, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);

            $pdf->SetXY($tab3_posx+91, $tab3_top+$y);
            $pdf->MultiCell(25, 3, price($totalAmountTTC, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
            
            $pdf->SetXY($tab3_posx+116, $tab3_top+$y);
            $pdf->MultiCell(25, 3, price($totalAmountCollection, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);

            $pdf->SetXY($tab3_posx+141, $tab3_top+$y);
            $pdf->MultiCell(25, 3, price($totalCollectionTVA, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);

            $pdf->SetXY($tab3_posx+166, $tab3_top+$y);
            $pdf->MultiCell(25, 3, price($totalCollectionTTC, 0, $outputlangs, 1, 2, 2), 0, 'R', 0);
    ...
    private function getSalesListForInvoice($fk_invoice)
    {
        $sql = "SELECT ";
        ...
        $sql.= " WHERE  fk_seller_invoice=". $fk_invoice . " AND status IN(1)";
