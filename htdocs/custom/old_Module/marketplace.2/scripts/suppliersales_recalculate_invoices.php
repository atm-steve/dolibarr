...
if (! isset($argv[1])) {	// Check parameters
    print "Usage: ".$script_file." username \n";
    exit(-1);
}
print '--- start'."\n";
...
$sql = "SELECT rowid, fk_user_author";
$sql.= " FROM ".MAIN_DB_PREFIX."facture";
$sql.= " WHERE paye=1 AND type=0 AND rowid not in (SELECT fk_customer_invoice FROM ".MAIN_DB_PREFIX."marketplace_sales) ";
    ...
    if ($num) {
        while ($i < $num) {
            ...
            if ($obj) {
                ...
                $fuser = new User($db);
                $fuser->fetch($obj->fk_user_author);

                $ret = $mpSales->processInvoice($fuser, $invoice, true);
                if ($ret > 0) {
                    $salesCreated += $ret;
                    $nbUpdated++;
                }
                if (count($mpSales->errors)) {
                    foreach ($mpSales->errors as $errorMsg) {
                        print $errorMsg ."\n";
                    }
                }
            }
...
if (! $error) {
    $db->commit();
    print '--- end ok'."\n";
    print $num . " invoices need an update \n";
    print $nbUpdated . " invoices updated \n";
    print $salesCreated . " sales created \n";
} else {
