...
var Ticket = jQuery.Class({
        ...
        for (var i in this.lines) {
            ...
            if (parseFloat(line["remise_percent_global"]) != parseFloat(discount)) {
                ...
                if(this.lines[i].localtax1_tx==0) {
                    ...
                    this.lines[i].total_ttc = this.lines[i].total_ht + ((this.lines[i].total_ht * parseFloat(this.lines[i].tva_tx))/100) + ((this.lines[i].total_ht * parseFloat(this.lines[i].localtax1_tx))/100);
                    sum = parseFloat(sum) + Math.round(parseFloat(this.lines[i].total_ttc) * 100) / 100;
                }
    ...
    addLine: function (idProduct, add) {
        ...
        if (_TPV.infoProduct == 0 || typeof add != 'undefined') {
            ...
            if (this.getLine(idProduct) != undefined) {
                ...
                if (_TPV.products[idProduct]["stock"] > this.getLine(idProduct).cant || _TPV.products[idProduct]["stock"] == "all") {
                    ...
                    if (idBalanza.length > 0 && idBalanza.substring(0, 2) == _TPV.defaultConfig['module']['barcode_flag'] && product.is_bulk == "1") {
                        // If price is used into barcode
                        if (_TPV.defaultConfig['module']['barcode_price'] == "1") {
                            var priceBalanzaEuros = idBalanza.substring(7, 9);
                            var priceBalanzaCts = idBalanza.substring(9, 12);

                            var priceBalanza = priceBalanzaEuros.concat('.', priceBalanzaCts);

                            var qtyBlanza = parseFloat(priceBalanza) / parseFloat(this.getLine(idProduct).price_ttc);
                            //product["cant"] = parseFloat(qtyBlanza).toFixed(3);
                            var newQty = this.getLine(idProduct).cant + parseFloat(qtyBlanza);
                            this.getLine(idProduct).cant = newQty.toFixed(3);
                        } else {
                            // barcode contains weight indication
        ...
        if (_TPV.products[idProduct]["is_bulk"] == 1) {
            this.editTicketLine(idProduct);
        }
    ...
    editTicketLine: function (idProduct) {
        ...
        $('#line_quantity').focus();
        $('#line_quantity').select();
        $('#id_btn_editTicketline').unbind('click');

        /* Librethic : after editing qty line, press Enter to validate input */
        $("#line_quantity").keydown(function (e) {
            if (e.keyCode == 13) {
                e.keyCode = ''; // Init keycode to avoid conflict
        ...
        $('#id_btn_editTicketline').click(function () {
            if (_TPV.ticket.checkApplyQuantity(idProduct, $('#line_quantity').val())) {
                ...
                // librethic: can do calculation with quantity field
                var qtyline = $('#line_quantity').val();
                var stringtoeval = qtyline.indexOf('+');
                if (stringtoeval > 0) {
                    qtyline = parseFloat(eval(qtyline));
                }
                line.setQuantity(qtyline);
    ...
    sendTicket: function () {
        ...
        if (_TPV.ticket.difpayment > 0){
            ...
            if(_TPV.points){
                if(_TPV.ticket.points==0 && _TPV.ticket.customerpay>=minreward) {
                    _TPV.points = parseFloat(_TPV.points)+parseFloat(_TPV.ticket.customerpay);
                }
                else{
                    _TPV.points = parseFloat(_TPV.points)-parseFloat(_TPV.ticket.points);
                    if(_TPV.ticket.customerpay>=minreward) {
                        _TPV.points = parseFloat(_TPV.points) + parseFloat(_TPV.ticket.customerpay);
                    }
                }
            }
            ...
            if (result[0]["stock"] == "all" || result[0]["stock"] > 0) {
                ...
                if (idBalanza.length > 0 && idBalanza.substring(0, 2) == _TPV.defaultConfig['module']['barcode_flag'] && product.is_bulk == "1" ) {
                    ...
                    if (_TPV.defaultConfig['module']['barcode_price'] == "1") {
                        ...
                        var qtyBlanza = parseFloat(priceBalanza) / parseFloat(product.price_ttc);;
                        product["cant"] = parseFloat(qtyBlanza).toFixed(3);
                        this.cant = parseFloat(qtyBlanza).toFixed(3);
                       
                        product["price_ttc"] = priceBalanza;
...
var TPV = jQuery.Class({
    ...
    searchProduct: function () {
        ...
        if (data['search'].length > 0 && data['search'].substring(0, 2) == _TPV.defaultConfig['module']['barcode_flag']) {
            var ref = data['search'].substring(2, 7);
            data['search'] = ref;
        }
        ...
        $('#id_selectProduct').append(
            $('<option></option>').val(0).html('Produits ' + result.length)
        );
        $.each(result, function (id, item) {
                if (_TPV.defaultConfig['module']['ttc'] == 0) {
					$('#id_selectProduct').append(
