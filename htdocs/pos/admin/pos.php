...
if (GETPOST("action") == 'set') {
	...
	if (GETPOST("POS_FACTURE", "int") == 0 && $conf->global->POS_TICKET == 0) {
		...
		$res = dolibarr_set_const($db, "POS_TICKET_LOGO", GETPOST("POS_TICKET_LOGO"), 'chaine', 0, '', $conf->entity);
		...
		$res = dolibarr_set_const($db, "POS_BARCODE_FLAG", GETPOST("POS_BARCODE_FLAG"), 'chaine', 0, '', $conf->entity);

		if ((!$res) > 0) {
			$error++;
        }
        
        $res = dolibarr_set_const($db, "POS_BARCODE_WITH_PRICE", GETPOST("POS_BARCODE_WITH_PRICE"), 'chaine', 0, '', $conf->entity);

		if ((!$res) > 0) {
			$error++;
		}

		$res = dolibarr_set_const($db, "POS_SHOWHIDE_CATEGORY", GETPOST("POS_SHOWHIDE_CATEGORY"), 'chaine', 0, '', $conf->entity);

		if ((!$res) > 0) {
			$error++;
        }
        
        $res = dolibarr_set_const($db, "POS_CATEGORIE_BULK", GETPOST("POS_CATEGORIE_BULK"), 'chaine', 0, '', $conf->entity);
...
$var = !$var;
print '<tr ' . $bc[$var] . '><td>';
print $langs->trans("ShowTicketLogo");
print '<td colspan="2">';
print $html->selectyesno("POS_TICKET_LOGO", $conf->global->POS_TICKET_LOGO, 1);
print "</td></tr>\n";
...
if (version_compare(DOL_VERSION, 8.0) >= 0 && ! empty($conf->category->enabled))
{
	$var = !$var;
	print '<tr ' . $bc[$var] . '><td>';
    print $langs->trans("PosCategoryBulk");
    print '<td colspan="2">';
    print '<input type="text" name="POS_CATEGORIE_BULK" size="3" value="'. $conf->global->POS_CATEGORIE_BULK .'"/>';
	//print $html->selectyesno("POS_CATEGORIE_BULK", $conf->global->POS_CATEGORIE_BULK, 1);
	print "</td></tr>\n";
}
...
$var = !$var;
print '<tr ' . $bc[$var] . '>';
print '<td>' . $langs->trans("POSBarCodeFlagWithPrice") . '</td>';
print '<td colspan="2">';
print $html->selectyesno("POS_BARCODE_WITH_PRICE", $conf->global->POS_BARCODE_WITH_PRICE, 1);
print '</td>';
//print '<td><input type="text"  maxlength="2" class="flat" name="POS_BARCODE_WITH_PRICE" value="' . ($_POST["POS_BARCODE_WITH_PRICE"] ? $_POST["POS_BARCODE_WITH_PRICE"] : $conf->global->POS_BARCODE_WITH_PRICE) . '" size="4"> </td>';
print '</tr>';
