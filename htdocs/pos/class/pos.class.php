...
class POS extends CommonObject
{
	...
	public static function getProductbyId($idProd, $idCust)
	{
        ...
        // If product si in 'bulk' category then set flag 'is_bulk'
        if (!class_exists('Categorie')) {
            require_once DOL_DOCUMENT_ROOT . '/categories/class/categorie.class.php';
        }
        $catStatic = new Categorie($db);
        $categories = $catStatic->getListForItem($idProd, 'product', 's.rowid', 'ASC', 5);
        if (is_array($categories) and count($categories) > 0) {
            foreach ($categories as $catProduct) {
                if ($catProduct['id'] == $conf->global->POS_CATEGORIE_BULK) {
                    $ret[0]["is_bulk"] = 1;
                    break;
                }
            }
	...
	public static function SearchProduct(
        ...
        if (substr($idSearch,0,2)== $conf->global->POS_BARCODE_FLAG){
	...
	public static function getHistoric(
		...
		//mysql strict
		$sql .= ', f.ticketnumber, f.total_ttc, f.date_closed, f.fk_user_close, f.date_creation, f.fk_statut, f.customer_pay, f.difpayment, f.fk_place, s.nom, s.rowid, t.name, f.fk_cash, f.type';
		//
	...
	public static function getHistoricFac(
		...
		//mysql strict
		$sql .= ' f.facnumber, f.total_ttc, f.fk_user_valid, f.datec, f.fk_statut, pf.fk_place, s.nom, s.rowid, u.firstname, u.lastname, t.name, pf.fk_cash, f.type';
		//
		...
		$sql .= ' GROUP BY p.rowid,';
		//mysql strict
		$sql .= ' p.ticketnumber, p.total_ttc, p.fk_user_close, p.date_creation, p.fk_statut, p.fk_place, s.nom, s.rowid, u.firstname, u.lastname, t.name, p.fk_cash, p.type';
		//
	...
	public static function getConfig()
	{
        ...
        $ret['data']['module']['barcode_price'] = $conf->global->POS_BARCODE_WITH_PRICE ? $conf->global->POS_BARCODE_WITH_PRICE : 0;
	...
	public static function get_prod($idCat, $more, $ticketstate)
	{
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$objs[$objp->ref]["label"] = $objp->label;
				$objs[$objp->ref]["description"] = $objp->description;
