	...
	if ($num > 0)
	{
		...
		while ($i < $num)
		{
			...
			$i++;
				...
				$xmlfile .= '<client>';
			    $xmlfile .= '<reference>'.$objp->facnumber.'</reference>';
			    $xmlfile .= '<numero>'.dol_trunc($objp->numero).'</numero>';
			    $xmlfile .= '<nom>'.dol_trunc($objp->nom,28).'</nom>';
			    $xmlfile .= '<modereglement>'.$objp->mode_rglt.'</modereglement>';
			    $xmlfile .= '<date>'.dol_print_date($db->jdate($objp->df),'day').'</date>';
			    $xmlfile .= '<montantht>'.price($objp->total_ht).'</montantht>';
			    $xmlfile .= '<taxes>'.price($objp->total_tva).'</taxes>';
			    $xmlfile .= '<montantttc>'.price($objp->total_ttc).'</montantttc>';
			    $xmlfile .= '<creance>'.price($objp->total_ttc-$objp->am-$cn-$deposit).'</creance>';
			    //$xmlfile .= '<etat>'.$facturestatic->LibStatut($objp->paye,$objp->fk_statut,5,$objp->am).'</etat>';
			    $xmlfile .= '</client>';
...
/*$xml2=simplexml_load_file("export.xml");
print_r($xml2);*/
/*
$fichier = 'export.xml';
$xml2 = simplexml_load_file($fichier);
echo "<table>";
foreach($xml2 as $client){
	echo "<tr>";
		echo '<td>'.$client->reference.'</td>';	
		echo '<td>'.$client->numero.'</td>';	
		echo '<td>'.$client->nom.'</td>';	
		echo '<td>'.$client->modereglement.'</td>';	
		echo '<td>'.$client->date.'</td>';	
		echo '<td>'.$client->taxes.'</td>';	
		echo '<td>'.$client->montantttc.'</td>';	
		echo '<td>'.$client->creance.'</td>';
	echo "</tr>";	
}
echo "</table>";
*/
...
/*
  ...
  $xml3 = simplexml_load_file('export.xml');
