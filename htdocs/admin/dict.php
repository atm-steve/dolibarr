...
//$tabsql[25]= "SELECT rowid   as rowid, label, type_template, private, position, nodoc, topic, content_lines, content, active FROM ".MAIN_DB_PREFIX."c_email_templates WHERE entity IN (".getEntity('email_template').")";
...
//$tabfield[25]= "label,type_template,private,position,nodoc,topic,content_lines,content";
...
//$tabfieldvalue[25]= "label,type_template,private,position,nodoc,topic,content_lines,content";
...
//$tabfieldinsert[25]= "label,type_template,private,position,nodoc,topic,content_lines,content,entity";
...
$tabhelp[25] = array(
		'topic' => $langs->trans('SeeSubstitutionVars'),
		'content' => $langs->trans('SeeSubstitutionVars'),
		'content_lines'=>$langs->trans('SeeSubstitutionVars'),
		...
		'nodoc' => $langs->trans("TemplateHasNoAttachedDoc"));
...
function fieldList($fieldlist, $obj='', $tabname='', $context='')
{
	...
	foreach ($fieldlist as $field => $value)
	{
		...
		elseif (in_array($fieldlist[$field], array('content')))
		{
			...
			{
				print '<td colspan="4"></td></tr><tr class="pair nohover"><td colspan="6">';		// To create an artificial CR for the current tr we are on
			}
			else print '<td>';
			if ($context != 'hide')
			{
		...
		else
		{
			...
			if ($fieldlist[$field]=='nododc') $class='maxwidth50';
