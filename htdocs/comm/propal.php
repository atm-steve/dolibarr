...
if (empty($reshook))
{
	...
	else if ($action == 'confirm_reopen' && $user->rights->propal->cloturer && ! GETPOST('cancel'))
	{
		...
		if ($object->statut == Propal::STATUS_SIGNED || $object->statut == Propal::STATUS_NOTSIGNED 
		|| $object->statut == Propal::STATUS_BILLED || $object->statut == Propal::STATUS_NOTFEASIBLE) {
	...
	if ($action != 'presend') {
		...
		if (empty($reshook))
		{
			if ($action != 'statut' && $action != 'editline')
			{
				...
				// ReOpen
				if (($object->statut == Propal::STATUS_SIGNED || $object->statut == Propal::STATUS_NOTSIGNED || $object->statut == Propal::STATUS_BILLED || $object->statut == Propal::STATUS_NOTFEASIBLE) && $user->rights->propal->cloturer) {
