  ...
  jQuery.fn.extend({
    ...
    updateEditableCell: function(callingElement) {
      ...
      if (settings.inputTypes) {
        var colSettings = settings.inputTypes.filter(function(input) {
          return input.column == columnIndex;
        })[0];
      }
      var inputField = getInputField(callingElement);
      var newValue = inputField.val();
      if (colSettings && colSettings.target) {
        let rowData = row.data();
        rowData[colSettings.target] = newValue;
        newValue = cell.data();
      }
      var returnValue;
    ...
    $(table.body()).on('click', 'td', function() {
      ...
      if ((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) {
        ...
        if (!$(cell).find('input').length && !$(cell).find('select').length && !$(cell).find('textarea').length) {
          ...
          $('#ejbeatycelledit').keypress(function(e) {
            if (e.which === 13) {
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
  ...
  switch (inputType) {
    ...
    case "datepicker-confirm":
      // Make sure jQuery UI is loaded on the page
