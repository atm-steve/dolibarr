...
class TExportComptaLd extends TExportCompta
{
	function __construct($db, $exportAllreadyExported = false, $addExportTimeToBill = false) {
		...
		$this->_format_ecritures_comptables_achat[0] = array(
				'name' => 'code_type_enregistrement',
				'length' => 2,
				'default' => 'E',
				'type' => 'text'
		);
		$this->_format_ecritures_comptables_achat[1] = array(
				'name' => 'code_journal',
				'length' => 2,
				'default' => 'AC',
				'type' => 'text'
		...
		$this->_format_ecritures_comptables_banque[0] = array(
				'name' => 'code_type_enregistrement',
				'length' => 2,
				'default' => 'E',
				'type' => 'text'
		);
		$this->_format_ecritures_comptables_banque[2] = array(
				'name' => 'code_journal',
				'length' => 2,
				'default' => 'BQ',
				'type' => 'text'
		...
		foreach ( $TabFactures as $id_facture => $infosFacture ) {
			...
			foreach ( $infosFacture['ligne_tiers'] as $code_compta => $montant ) {
				...
				$ligneFichier = array(
						...
						'DATH' => (! empty($objFacture->date_lim_reglement) ? $objFacture->date_lim_reglement : ''),
						'CPTA' => $code_compta,
						'libelle' => '"' . $tiers['nom'] . '"',
