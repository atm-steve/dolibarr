		...
		foreach ($TabFactures as $id_facture => $infosFacture) {
			...
			foreach($infosFacture['ligne_tiers'] as $code_compta => $montant) {
			    ...
			    $ligneFichier = array(
			    	...
			    	'CPTG'							=> $conf->global->EXPORT_COMPTA_GENERAL_CUSTOMER_ACCOUNT,
			    		'DATH'                      => !empty($db->jdate($objFacture->date_lim_reglement))? date('Ymd', $db->jdate($objFacture->date_lim_reglement) ) : '',
					'CPTA'                          => $code_compta,
					'libelle'						=> '"'.$tiers['nom'].'"',
			        'sens'							=> ($facture['type'] == 2 ? 'C' : 'D'),
			        'montant'						=> sprintf("%09.2f", number_format(abs($montant),2,'.','') ),
			...
			foreach($infosFacture['ligne_produit'] as $code_compta => $montant) {
				...
				$ligneFichier = array(
					...
					'numero_piece'					=> $facture['ref'],
				    'numero_compte'					=> '',
					'DATH'                          => !empty($objFacture->date_lim_reglement)? date('Ymd', $db->jdate($objFacture->date_lim_reglement) ) : '',
					'CPTG'							=> $code_compta,
					'CPTA'							=> '',
					'libelle'						=> '"'.$tiers['nom'].'"',
				    'sens'							=> $sens,
