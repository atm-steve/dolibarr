...
/**
 * Parent class for accounting exports; it is intended to be extended for vendor-specific exports
 */
class TExportCompta extends TObjetStd
{
	 ...
	 * @param bool $exportAllreadyExported  Whether to export elements that were already exported
	 * @param bool $addExportTime  Whether to set the "exported on" date on the exported invoices (extrafield)
	 */
	public function __construct(&$db, $exportAllreadyExported = false, $addExportTime = false)
	{
		...
		$this->TLogiciel = array(
			...
			,'sage30' => 'Sage 30'
			,'sage1000' => 'Sage 1000'
			,'sage1000_Koesio' => 'Sage 1000 Koesio'
			,'ciel' => 'Ciel'
			,'opensi' => 'Open SI'
			,'etag' => 'eTag'
			,'cegid' => 'Cegid'
		...
		if (!empty($conf->banque->enabled)) {
			$this->TTypeExport['ecritures_comptables_banque'] = 'Écritures comptables banque';
			$this->TTypeExport['infos_paiement'] = 'infos_paiement';
		}
		...
		$this->hookmanager->initHooks(array('exportcomptadao'));
	 ...
	 */
	public function init_plan_comptable()
	{
	 ...
	 *
	 * @param string $dt_deb  start date
	 * @param string $dt_fin  end date
	 */
	public function get_factures_client($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
		...
		// Hook to replace or alter $sql
		$parameters = array(
			'sql' => &$sql,
			'dt_deb' => $dt_deb,
			'dt_fin' => $dt_fin,
		);
		$reshook=$this->hookmanager->executeHooks('setSql', $parameters, $this, $action);    // Note that $action and $object may have been modified by some hooks
		if ($reshook < 0) {
			var_dump($this->hookmanager->errors, $this->hookmanager->error); exit;
		}
...
//			$facture->thirdparty->fetch_optionals();
			...
			if (in_array($facture->type, array(Facture::TYPE_SITUATION, Facture::TYPE_CREDIT_NOTE)) && ! empty($facture->situation_cycle_ref)) {
				foreach ($facture->lines as &$ligneSituation) {
					...
					} elseif (!empty($produit->array_options['options_code_tva'])) {
	 ...
	 * @param Product $produit  Product
	 * @return int|mixed|void
	 */
	public static function _get_code_compta_product(&$facture, &$produit)
	{
		global $conf;
		...
		// Cas de la France
		elseif ($facture->thirdparty->country_code == 'FR') {
		...
		// Cas de la vente CEE
		elseif ($facture->thirdparty->isInEEC()) {
	 ...
	 * @return int|void
	 */
	public static function get_code_compta_by_categ_tiers(&$facture, &$produit)
	{
	...
	/**
	 * @param array $TFactures
	 */
	public static function equilibreFacture(&$TFactures)
	{
	 ...
	 * @return string
	 */
	public static function _get_mode_reglement_label($id)
	{
	 ...
	 * @return array|void
	 */
	public function get_factures_fournisseur($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
		...
		while ($obj = $db->fetch_object($resql)) {
			...
			foreach ($facture->lines as $ligne) {
				...
				if (empty($codeComptableProduit)) {
					...
					} elseif ($ligne->fk_product_type == 1) {
				...
				} elseif (!empty($produit->array_options['options_code_tva_achat'])) {
	 ...
	 * @return array
	 */
	public function get_notes_de_frais($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
	 ...
	 * @param string $dt_deb
	 * @param string $dt_fin
	 * @return array  id => associative array representing an expense report
	 */
	public function get_notes_de_frais_ndfp($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
	 ...
	 * @return string
	 */
	public function get_code_comptable($fk_soc)
	{
	 ...
	 * @param string $dt_deb
	 * @param string $dt_fin
	 * @return array  Associative array: id of payment => associative array representing a payment
	 */
	public function get_reglement_tiers($dt_deb, $dt_fin)
	{
		global $db, $conf;
	 ...
	 * @param string $dt_deb
	 * @param string $dt_fin
	 * @return array  Associative array: code => associative array representing a product
	 */
	public function get_produits($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
	 ...
	 * @param string $dt_deb
	 * @param string $dt_fin
	 * @return array  Associative array: code (string) => associative array representing a third party
	 */
	public function get_tiers($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
		...
		$extrafields->fetch_name_optionals_label($socstatic->table_element);
		$extrafieldsSelectSQL = '';
		$socextrafields = $extrafields->attributes[$socstatic->table_element];
		foreach ($socextrafields['label'] as $efCode => $efLabel) {
			// skip separators
			if ($socextrafields['type'][$efCode] === 'separate') continue;
			$extrafieldsSelectSQL .= ', ex.' . $efCode . ' AS soc_ext_' . $efCode;
		}
		...
		if ((float) DOL_VERSION>=3.7) {
			...
			,s.fax
			,s.email
			,s.tms,rglt.code as mode_reglement_code
			,p.code as 'code_pays'
			,p.label as 'pays'
			,s.siret
			, rib.label as 'rib_label'
			, rib.code_banque
			, rib.code_guichet
			, rib.number as 'compte_bancaire'
			, rib.cle_rib
			, rib.bic
			, rib.iban_prefix as 'iban'
			, rib.domiciliation
			, rib.proprio as 'rib_proprio'
			, ex.fk_soc_affacturage, s.note_public
			, soctype.libelle AS soctype_libelle
			, s.ape AS ape
			, s.tva_intra AS tva_intra
			, rglt.code AS rglt_code
			, cond_rglt.libelle AS cond_rglt_libelle
			, cond_rglt.code AS cond_rglt_code
			, rib.bank AS libelle_banque
			, s.entity AS entity
			$extrafieldsSelectSQL
			...
			LEFT JOIN ".MAIN_DB_PREFIX."c_forme_juridique AS soctype ON soctype.code = s.fk_forme_juridique
			LEFT JOIN ".MAIN_DB_PREFIX."c_payment_term AS cond_rglt ON cond_rglt.rowid = s.cond_reglement
		...
		while ($obj = $db->fetch_object($resql)) {
			...
			// spécifique koesio: il faut aussi sélectionner le premier contact actif du tiers
			$socstatic->id = $obj->socid;
			$row['contacts'] = $socstatic->liste_contact() ?: array();
			// malheureusement, liste_contact() ne récupère pas le numéro de téléphone ou le fax,
			// donc autant fetcher le contact au complet…
			foreach ($row['contacts'] as $contactArray) {
				$contact = new Contact($db);
				if ($contact->fetch($contactArray['id']) > 0) {
					$contactArray['phone'] = $contact->phone_perso;
					$contactArray['fax'] = $contact->fax;
				}
			}
	...
	/**
	 * @param string $dt_deb  start date in database format
	 * @param string $dt_fin  end date in database format
	 * @return array
	 */
	public function get_banque($dt_deb, $dt_fin)
	{
		global $db, $conf, $user;
		...
		foreach ($TIdBank as $idBank) {
			...
			foreach ($links as $key => $val) {
				...
				if ($lineType == 'company') {
					...
					} elseif (in_array($bankline->label, array('(SupplierInvoicePayment)', 'Règlement fournisseur'))) {
	...
	/**
	 * @param array $infosBank  Associative array
	 * @return array
	 */
	public function getODVATTransfer($infosBank)
	{
		global $conf;
	...
	/**
	 * @param array[] $format  Array of associative arrays, each describing a column:
	 *                         - name (string)
	 *                         - length
	 *                         - type_value (either Data, PHP or hard-coded; in practice, most often "data")
	 *                         - value (string)
	 *                         - default
	 *                         - type (numeric, text, date, etc.)
	 *                         - format (for dates)
	 *                         - pad_type (left, right or both)
	 * @param $dataline
	 * @return string
	 */
	public function get_line(&$format, $dataline)
	{
		global $conf;
		...
		if (is_array($format) && count($format)>0) {
			foreach ($format as $fmt) {
				...
				if ($fmt['type_value'] == 'php') {
					// TODO: utiliser dol_eval, légèrement moins unsafe car interdit (en principe) l'accès
					//       au système de fichiers ou bien supprimer la feature si elle n'est pas utilisée
					//       ou si elle est remplaçable
					$valeur = eval('return '.strtr($fmt['value'], array(
						'@val@'=>$valeur
					)).';');
				} elseif ($fmt['type_value'] == 'dur') {
					$valeur = $fmt['value'];
				}
				...
				} elseif ($fmt['type'] == 'numeric' && !empty($fmt['format'])) {
				...
				} elseif (iconv_strlen($valeur) > $fmt['length']) {
	 ...
	 * @param string $dt_fin
	 */
	public function get_infos_paiement($dt_deb, $dt_fin)
	{
		global $langs;
		$TInfosPaiement = array();
		$sqlnum = 0;

		$dateFieldForFilter = 'tms';
		$fieldsForSelect = array(
			'rib.rowid       AS rowid',
			'rib.rum         AS reference_mandat',
			'rib.date_rum    AS date_signature_mandat',
			'cli.town        AS lieu_signature',
			'cli.code_compta AS code_client',
			'rib.iban_prefix AS code_iban',
			'rib.bic         AS code_bic',
			'rib.ending_date AS date_fin_mandat',
		);

		$sql = /* @lang SQL */
			"SELECT "
			. " " . implode(', ', $fieldsForSelect)
			. " FROM " . MAIN_DB_PREFIX . "societe_rib AS rib"
			. " INNER JOIN " . MAIN_DB_PREFIX . "societe AS cli ON rib.fk_soc = cli.rowid"
			. " WHERE"
			. "     rib.{$dateFieldForFilter} BETWEEN '{$dt_deb}' AND '{$dt_fin}'"
			. " ORDER BY rowid ASC";
		...
		while ($obj = $this->db->fetch_object($resql)) {
			$infosPaiement = get_object_vars($obj);
			// convert dates to timestamp
			// NOTE: aliases must start with 'date_' for this to work
			foreach ($infosPaiement as $k => &$v) {
				if ($v && preg_match('/^date_/', $k)) $v = $this->db->jdate($v);
			}
			$TInfosPaiement[$obj->rowid] = $infosPaiement;
		}

		return $TInfosPaiement;
	}

	/**
	 * Retourne le détail d'une configuration du module export compta
	 * @param array[] $TDataConf  Each array represents the configuration of a column of the export.
	 *                            Keys of the arrays inside TDataConf:
	 *                            - name (string)
	 *                            - length
	 *                            - type_value (either Data, PHP or hard-coded; in practice, most often "data")
	 *                            - value (string)
	 ...
	 * @return array|0  0 = column not found; array representing the column if found
	 */
	public function get_line_conf($TDataConf, $name)
	{
	...
	/**
	 * Supprimer les accents encodés en tant qu'entités HTML
	 *
