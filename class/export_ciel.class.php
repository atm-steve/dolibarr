...
class TExportComptaCiel extends TExportCompta
{
	...
	function get_file_ecritures_comptables_ventes($format, $dt_deb, $dt_fin) {
		...
		if (empty($format)) {
			$format = $this->_format_ecritures_comptables_vente;
		}

		$TabFactures = parent::get_factures_client($dt_deb, $dt_fin);

		$type = 'M';
		$TformatCodeJournal=parent::get_line_conf($format,'code_journal');
		if (array_key_exists('default', $TformatCodeJournal) && !empty($TformatCodeJournal['default'])) {
			$codeJournal =$TformatCodeJournal['default'];
		} else {
			$codeJournal = 'VE';
		}
		...
		foreach ( $TabFactures as $id_facture => $infosFacture ) {
			...
			// Lignes TVA
			if (is_array($infosFacture['ligne_tva']) && count($infosFacture['ligne_tva']) > 0) {
	...
	function get_file_ecritures_comptables_achats($format, $dt_deb, $dt_fin) {
		...
		if (empty($format))
			$format = $this->_format_ecritures_comptables_achat;
	...
	function get_file_reglement_tiers($format, $dt_deb, $dt_fin) {
		...
		if (empty($format))
			$format = $this->_format_reglement_tiers;
	...
	function get_file_ecritures_comptables_banque($format, $dt_deb, $dt_fin) {
		...
		if (empty($format))
			$format = $this->_format_ecritures_comptables_banque;
