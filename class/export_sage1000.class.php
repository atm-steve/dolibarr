<?php
/*************************************************************************************************************************************************
 * Format d'export comptable Sage 1000
 *************************************************************************************************************************************************/
dol_include_once("/exportcompta/class/export.class.php");
class TExportComptaSage1000 extends TExportCompta
{
	function __construct(&$db, $exportAllreadyExported = false, $addExportTime = false) {
		...
		// if ($conf->agefodd->enabled && $conf->global->EXPORT_COMPTA_ADD_AGEFODD_PRESTA) $this->_format_ecritures_comptables_vente[] = array('name' => 'type_prestation', 'length' => 1, 'default' => '', 'type' => 'text');
		$this->_format_ecritures_comptables_vente[] = array(
				'name' => 'fixe',
				'length' => 5,
				'default' => '1',
	...
	function get_file_ecritures_comptables_ventes($format, $dt_deb, $dt_fin) {
		...
		if (empty($compte_general_client))
			$compte_general_client = '41100000';

		if ($conf->global->EXPORTCOMPTA_BLOCK_IF_NOACCOUNT) {
			foreach ( $TabFactures as $id_facture => $infosFacture ) {
				$facture = &$infosFacture['facture'];
				if ($facture['mode_reglement_code'] !== "VIR" && $facture['mode_reglement_code'] !== "CHQ") {
					exit('Le mode de réglement est incorrecte sur la facture ' . $facture['ref'] . ', chéque ou virement uniquement');
				}
			}
		}
		...
		foreach ( $TabFactures as $id_facture => $infosFacture ) {
			...
			$libelle = "FAC " . $tiers['nom'];
			if (! empty($facture['ref_client']))
				$libelle .= ' - ' . $facture['ref_client'];

			// Lignes client
			foreach ( $infosFacture['ligne_tiers'] as $code_compta => $montant ) {
				// var_dump($montant,$tiers, $facture);exit;
				$ligneFichier = array(
						'id_societe' => $tiers['ref'],
						'date_piece' => $facture['date'],
						'numero_piece' => $facture['ref'],
						'numero_compte_general' => $compte_general_client,
						'numero_tiers' => $tiers['code_compta'],
			...
			foreach ( $infosFacture['ligne_produit'] as $code_compta => $montant ) {
				...
				//Hack for panthera
				if ($conf->agefodd->enabled && !empty($conf->global->EXPORTCOMPTA_PANTHERA)) {
					$sql='SELECT catsoc.fk_soc FROM '.MAIN_DB_PREFIX.'categorie_societe as catsoc';
					$sql.=' INNER JOIN '.MAIN_DB_PREFIX.'categorie as cat ON cat.rowid=catsoc.fk_categorie';
					$sql.=' WHERE catsoc.fk_categorie='.$conf->global->EXPORTCOMPTA_PANTHERA.' AND catsoc.fk_soc='.$tiers['ref'];
	...
	function get_file_ecritures_comptables_achats($format, $dt_deb, $dt_fin) {
		...
		if (empty($compte_general_fournisseur))
			$compte_general_fournisseur = '40100000';
	...
	function get_file_ecritures_comptables_ndf($format, $dt_deb, $dt_fin) {
		...
		if (empty($compte_general_user))
			$compte_general_user = '40100000';
		...
		foreach ( $TabBank as $id_bank => $infosBank ) {
			...
			if (! empty($object)) {
				if ($object->element == 'societe')
					$label = $object->name;
				if ($object->element == 'chargesociales')
					$label = $object->type_libelle;
				if ($object->element == 'user')
					$label = $object->firstname . ' ' . $object->lastname;
				if (get_class($object) == 'BonPrelevement')
					$label = $object->ref;
			}
			...
			if (in_array($bankline['label'], array(
	...
	function get_file_tiers($format, $dt_deb, $dt_fin) {
		...
		foreach ( $TabTiers as $code_compta => $tiers ) {
			...
			$ligneFichier = array_merge($tiers,
					array(
